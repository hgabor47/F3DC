...
uses types,messages,forms,applicationfileio,math,controls,classes,SysUtils,dialogs,
  ...
  gltexture,glfile3ds,globjects,geometryBB,glWindowsfont, glmaterial,
...
//Classes
  ...
  public
    ...
    constructor Create(AOwner: TComponent;fullviewer:TGLFullscreenviewer); overload;
...
//  exCadencer:=TGLCadencer.Create(self);
  exCadencer:=TGLCadencer.Create(AOWner);
  exCadencer.FixedDeltaTime:=0.01;
  Setvector(ScaleBase,0.1,0.1,0.1);

  View := TGLFullscreenviewer.Create(AOwner);
  ...
  cam.Position.Z:=33;
  view.Camera:=cam;
  with view.Camera.AddNewChild(TGLLightSource) do
  begin
      name := 'headlight';
  end;
  with view.Camera.AddNewChild(TGLLightSource) do
  ...
  with exCadencer {TGLCadencer.Create(AOwner)} do
  begin
      scene:=glscene;
      sleeplength:=1;
      enabled:=true;
  ...
  view.Buffer.BackgroundColor:=$bbbbbb;
  Viewer := View;
  view.Active:=true;
end;

constructor TForm3d.Create(AOwner: TComponent;fullviewer:TGLFullscreenviewer);
begin
     randomize;
     inherited Create (AOwner);
     controlslist:=tlist.create;
...
procedure TForm3d.BuildEnvironment(mode:TBEmode;basecolor:Tcolor=$00AC937D;bgcolor:Tcolor=$00AC937D;colordistance:single=0.1;density:integer=80;skydome:boolean=true;envdistance:integer=1000);
var glscene:TGLScene;
    i,j:integer;
    l:TGLLines;
    glcolor:TGLColor;
