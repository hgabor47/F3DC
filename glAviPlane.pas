...
type
    ...
    private
        ...
        fFirstFrameSound,fLastFrameSound:integer;
        fUpdateRate: integer;
        pAvi: IAVIFile;
        ...
        AviInfoSound: TAVISTREAMINFOA;
        AviInfo: TAVISTREAMINFOA;

        pFrame: IGetFrame;
        pFrameSound: IGetFrame;
    ...
    public
        ...
        function GetFrameSound(Framenum: integer): boolean;
...
procedure TGLAviPlane.buildlist(var rci: TRenderContextInfo);
var
    w, h: integer;
    x, y: integer;
    r: integer;
        ...
        rmTriStrip:
            begin
                ...
                begin
                    ...
                    begin
                        ran:=0;//sin((x*0.06)+(li*0.063));//-(rgb^.rgbtred / 255)*4;//
                        glcolor4f(rgb^.rgbtred / 255, rgb^.rgbtgreen / 255, rgb^.rgbtblue / 255,0.5+(ran*2));
                        glvertex3f(x / (60 / r), y / (60 / r), ran);
                        glvertex3f(x / (60 / r), (y - 1) / (60 / r), ran);
                        inc(rgb, r);
                    end;
                    if r > 1 then
                        inc(rgb, w * (r - 1));
...
function TGLAviPlane.GetFrameSound(Framenum: integer): boolean;
var
...
begin
    ...
    try
        tempbmi := AVIStreamGetFrame(pFrameSound, Framenum);
...
procedure tglAviPLane.LoadAudio;
var len,plen:integer;
    p:pbyte;
    p2:PWAVEFORMATEX;
    res:HResult;
    mem:tfilestream;
begin
    //BAD !!!
    fFirstFrameSound := AVIStreamStart(pavisound); // 0
    fLastFrameSound := AVIStreamEnd(pavisound); // 1455360  stream length
    res := AVIStreamReadFormat(pavisound, fFirstFrameSound, nil, @len); // n = 18
    getmem(p2,len);
    res := AVIStreamReadFormat(pavisound, fFirstFrameSound, p2, @len); // n = 18
    res:=AVIStreamLength(pavisound);
    res := AVIStreamRead(pavisound, 0, AVISTREAMREAD_CONVENIENT, nil, 0, @len, nil); //53760
    getmem(p,len);
    plen:=len;
    res := AVIStreamRead(pavisound, 0, AVISTREAMREAD_CONVENIENT, nil, 0, @len, nil);
    mem:=tfilestream.Create('w.wav',fmCreate);
    mem.Write(p^,plen);
    mem.Destroy;
    freemem(p);
    //AVIStreamReadFormat(pavisound,lWaveBeginTime, &wf, &lSize);
end;
...
begin
    ...
    result := AVIFILEGetStream(pavi, pavisound, streamtypeAUDIO, 0) = AVIERR_OK;
    if not result then
    begin
        nosound:=true;
    end;
    ...
    if not nosound then
    begin
         result := AVIStreamInfo(pavisound, AviInfoSound, sizeof(TAVISTREAMINFOA)) = AVIERR_OK;
         if not result then
            exit;
    end;
...
begin
    ...
    begin
        ...
        begin
            ...
            colorArray[(y * (w + 2)) + x][0] := rgb^.rgbtRed / 256;
            colorArray[(y * (w + 2)) + x][1] := rgb^.rgbtGreen / 256;
            colorArray[(y * (w + 2)) + x][2] := rgb^.rgbtBlue / 256;
            if rendermode = rmTriStripArray then
            begin
                if i = 1 then
                    dec(rgb, (rw * (r - 1)));
